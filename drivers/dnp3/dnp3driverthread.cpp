...
void Dnp3DriverThread::run()
{
	IT_IT("Dnp3DriverThread::run");

	IT_COMMENT("Dnp3DriverThread Running");
...
bool Dnp3DriverThread::StartProcess(char* pCommandLine, char* pWorkingDir)
{ 
...
/*
PROCESS_INFORMATION* Dnp3DriverThread::getProcInfo(void)
{
    return(pProcInfo);
}
*/

void Dnp3DriverThread::EndProcess(int nIndex)
{	
	if(pProcInfo[nIndex].hProcess)
	{
		int nPauseEnd = 100;
...
int Dnp3DriverThread::send_ack_to_child(int address, int data, char* pipeName)
{
...
int Dnp3DriverThread::pipe_put(char* pipe_name, char *buf, int len)
{ 
	int rc = 0;
...
void Dnp3DriverThread::TerminateProtocol() // parent requests the thread close
{
	IT_IT("Dnp3DriverThread::TerminateProtocol");
