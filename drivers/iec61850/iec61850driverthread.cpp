...
void Iec61850DriverThread::run()
{
	IT_IT("Iec61850DriverThread::run");

	IT_COMMENT("Iec61850DriverThread Running");

    ///////////////////start child process iec61850master.exe////////////////////
	char line_number[50];
	...
	UnitFail("IEC61850 driver stopped");

    Done = true;
}


bool Iec61850DriverThread::StartProcess(char* pCommandLine, char* pWorkingDir)
{ 
...
/*
PROCESS_INFORMATION* Iec61850DriverThread::getProcInfo(void)
{
    return(pProcInfo);
}
*/

void Iec61850DriverThread::EndProcess(int nIndex)
{	
	if(pProcInfo[nIndex].hProcess)
	{
		int nPauseEnd = 100;
...
int Iec61850DriverThread::send_ack_to_child(int address, int data, char* pipeName)
{
...
int Iec61850DriverThread::pipe_put(char* pipe_name, char *buf, int len)
{ 
	int rc = 0;
...
void Iec61850DriverThread::TerminateProtocol() // parent requests the thread close
{
	IT_IT("Iec61850DriverThread::TerminateProtocol");
