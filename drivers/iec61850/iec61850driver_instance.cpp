...
void Iec61850driver_Instance::Start() 
{
	...
	State = STATE_RESET;
	QString cmd = "select * from UNITS where UNITTYPE='iec61850driver' and NAME in(" + DriverInstance::FormUnitList()+ ");";
...
*Returns:none
*/
void Iec61850driver_Instance::Stop() // stop everything under this driver's control
{	
	IT_IT("Iec61850driver_Instance::Stop");
...
*Returns:none
*/
void Iec61850driver_Instance::QueryResponse(QObject *p, const QString &c, int id, QObject*caller) // handles database responses
{
	IT_IT("Iec61850driver_Instance::QueryResponse");
...
//						Values[k].fSpotValue = a.toInt(); 
		...
		case tUnitProperties:
		{
			if(GetConfigureDb()->GetNumberResults() > 0)
			{
				...
				QTextIStream is(&s); // extract the values
				//
				is >> IecItems;	  // how many iec61850 items there are in the RTU or PLC
				is >> Cfg.SampleTime; // how long we sample for in milliseconds
				//IEC61850 TCP
				is >> Cfg.IEC61850ServerIPAddress;  // IEC61850 server IP address (slave)
				is >> Cfg.IEC61850ServerTCPPort;  // IEC61850 server TCP port
				...
				//Start IEC61850 client driver
				if(!Connect())
				{
					QSLogAlarm(Name,tr("Failed to start IEC61850 client driver"));
				}
			}
		}
		break;
		case tGetSamplePointNamefromIOA:
		{
			...
			{
				// 
				#ifdef DEPRECATED_IEC61850CONFIG
		...
		case tGetIOAfromSamplePointName:
		{
			...
			{
				// 
				#ifdef DEPRECATED_IEC61850MASTER_CONFIG
...
*/
//Realtime method
void Iec61850driver_Instance::driverEvent(DriverEvent *p)
{
	IT_IT("Iec61850driver_Instance::driverEvent");
...
//Realtime method
bool Iec61850driver_Instance::expect(unsigned int cmd)
{
	IT_IT("Iec61850driver_Instance::expect");
...
//Realtime method
void Iec61850driver_Instance::removeTransaction()
{
	IT_IT("Iec61850driver_Instance::removeTransaction");
...
void Iec61850driver_Instance::Tick()
{
	IT_IT("Iec61850driver_Instance::Tick");
...
void Iec61850driver_Instance::get_items_from_local_fifo(void)
{
	...
	{ 
		...
		if(State == STATE_FAIL)
		{
			...
			{
				QString msg;
				msg.sprintf("IEC61850 master on line %d is now connected to IEC61850 server.", instanceID + 1); 
				UnFailUnit(msg);
				State = STATE_ASK_GENERAL_INTERROGATION;
			}
		}
		...
		switch(p_item->iec_type)
		{
			...
			case C_LO_ST_1:
			{
				if(State != STATE_FAIL)
				{
					printf("IEC61850 master on line %d has lost connection with IEC61850 server...\n", instanceID + 1);

					QString msg;
					msg.sprintf("IEC61850 master on line %d has lost connection with IEC61850 server...", instanceID + 1); 
					FailUnit(msg);
...
//Realtime method
bool Iec61850driver_Instance::event(QEvent *e)
{
	IT_IT("Iec61850driver_Instance::event");
...
bool Iec61850driver_Instance::Connect() 
{	
	IT_IT("Iec61850driver_Instance::Connect");
	//return 0 on fail
	//return 1 on success

	if(pConnect) delete pConnect;
	
	pConnect = new Iec61850DriverThread(this); // create the connection
...
*Returns:none
*/
bool  Iec61850driver_Instance::Disconnect()        
{
	IT_IT("Iec61850driver_Instance::Disconnect");
...
*/
//Realtime method
bool  Iec61850driver_Instance::DoExec(SendRecePacket *t)
{
	IT_IT("Iec61850driver_Instance::DoExec");
...
*Returns:none
*/
void Iec61850driver_Instance::Command(const QString & name, BYTE cmd, LPVOID lpPa, DWORD pa_length, DWORD ipindex) // process a command for a named unit 
{
	IT_IT("Iec61850driver_Instance::Command");
...
{
	Iec61850driver_Instance * cl = (Iec61850driver_Instance*)recvCallBackParam;
...
void Iec61850driver_Instance::get_utc_host_time(struct cp56time2a* time)
{
	struct timeb tb;
	struct tm	*ptm;
