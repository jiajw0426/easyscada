...
void Opc_client_ae_DriverThread::run()
{
    ...
    ///////////////////start child process opc_client_ae.exe////////////////////
	char line_number[50];
	char polling_time[50];

	strcpy(pipe_name, "\\\\.\\pipe\\opc_client_ae_namedpipe");

	itoa(instanceID + 1, line_number, 10);

    strcat(pipe_name, line_number);

	itoa(((Opc_client_ae_Instance*)Parent)->Cfg.SampleTime, polling_time, 10);
		
	strcpy(pCommandLine, "C:\\scada\\bin\\opc_client_ae.exe -a ");
	strcat(pCommandLine, ((Opc_client_ae_Instance*)Parent)->Cfg.OpcServerIPAddress);
	strcat(pCommandLine, " -p ");
	strcat(pCommandLine, ((Opc_client_ae_Instance*)Parent)->Cfg.OpcServerProgID);
	strcat(pCommandLine, " -q ");
	strcat(pCommandLine, ((Opc_client_ae_Instance*)Parent)->Cfg.OpcclassId);
	...
	UnitFail("OPC client AE driver stopped");
...
/*
PROCESS_INFORMATION* Opc_client_ae_DriverThread::getProcInfo(void)
{
    return(pProcInfo);
}
*/

void Opc_client_ae_DriverThread::EndProcess(int nIndex)
{	
	if(pProcInfo[nIndex].hProcess)
	{
		int nPauseEnd = 100;
...
int Opc_client_ae_DriverThread::pipe_put(char* pipe_name, char *buf, int len)
{ 
	int rc = 0;
