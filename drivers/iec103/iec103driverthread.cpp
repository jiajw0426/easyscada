...
void Iec103DriverThread::run()
{
	IT_IT("Iec103DriverThread::run");
	...
	UnitFail("IEC 103 driver stopped");

    Done = true;
}


bool Iec103DriverThread::StartProcess(char* pCommandLine, char* pWorkingDir)
{ 
...
/*
PROCESS_INFORMATION* Iec103DriverThread::getProcInfo(void)
{
    return(pProcInfo);
}
*/

void Iec103DriverThread::EndProcess(int nIndex)
{	
	if(pProcInfo[nIndex].hProcess)
	{
		int nPauseEnd = 100;
...
int Iec103DriverThread::send_ack_to_child(int address, int data, char* pipeName)
{
...
int Iec103DriverThread::pipe_put(char* pipe_name, char *buf, int len)
{ 
	int rc = 0;
...
void Iec103DriverThread::TerminateProtocol() // parent requests the thread close
{
	IT_IT("Iec103DriverThread::TerminateProtocol");
