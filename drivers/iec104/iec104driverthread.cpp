...
void Iec104DriverThread::run()
{
	IT_IT("Iec104DriverThread::run");

	IT_COMMENT("Iec104DriverThread Running");

    ///////////////////start child process iec104master.exe////////////////////
	char line_number[50];
...
bool Iec104DriverThread::StartProcess(char* pCommandLine, char* pWorkingDir)
{ 
...
/*
PROCESS_INFORMATION* Iec104DriverThread::getProcInfo(void)
{
    return(pProcInfo);
}
*/

void Iec104DriverThread::EndProcess(int nIndex)
{	
	if(pProcInfo[nIndex].hProcess)
	{
		int nPauseEnd = 100;
...
int Iec104DriverThread::send_ack_to_child(int address, int data, char* pipeName)
{
...
int Iec104DriverThread::pipe_put(char* pipe_name, char *buf, int len)
{ 
	int rc = 0;
...
void Iec104DriverThread::TerminateProtocol() // parent requests the thread close
{
	IT_IT("Iec104DriverThread::TerminateProtocol");
