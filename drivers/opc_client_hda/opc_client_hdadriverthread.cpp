...
void Opc_client_hda_DriverThread::run()
{
    ...
    ///////////////////start child process opc_client_hda.exe////////////////////
	char line_number[50];
	char polling_time[50];

	strcpy(pipe_name, "\\\\.\\pipe\\opc_client_hda_namedpipe");

	itoa(instanceID + 1, line_number, 10);

    strcat(pipe_name, line_number);

	itoa(((Opc_client_hda_Instance*)Parent)->Cfg.SampleTime, polling_time, 10);
		
	strcpy(pCommandLine, "C:\\scada\\bin\\opc_client_hda.exe -a ");
	strcat(pCommandLine, ((Opc_client_hda_Instance*)Parent)->Cfg.OpcServerIPAddress);
	strcat(pCommandLine, " -p ");
	strcat(pCommandLine, ((Opc_client_hda_Instance*)Parent)->Cfg.OpcServerProgID);
	strcat(pCommandLine, " -q ");
	strcat(pCommandLine, ((Opc_client_hda_Instance*)Parent)->Cfg.OpcclassId);
	...
	UnitFail("OPC client HDA driver stopped");

    Done = true;
}


bool Opc_client_hda_DriverThread::StartProcess(char* pCommandLine, char* pWorkingDir)
{ 
...
/*
PROCESS_INFORMATION* Opc_client_hda_DriverThread::getProcInfo(void)
{
    return(pProcInfo);
}
*/

void Opc_client_hda_DriverThread::EndProcess(int nIndex)
{	
	if(pProcInfo[nIndex].hProcess)
	{
		int nPauseEnd = 100;
...
int Opc_client_hda_DriverThread::send_ack_to_child(int address, int data, char* pipeName)
{
...
int Opc_client_hda_DriverThread::pipe_put(char* pipe_name, char *buf, int len)
{ 
	int rc = 0;
...
void Opc_client_hda_DriverThread::TerminateProtocol() // parent requests the thread close
{
	IT_IT("Opc_client_hda_DriverThread::TerminateProtocol");
