...
inline int map_type(dbFieldDescriptor* fd) { 
    return (fd->type < dbField::tpArray) 
        ? gb2cli_type_mapping[fd->type] 
        : (fd->type == dbField::tpArray && fd->components->type < dbField::tpArray)
          ? cli_array_of_oid + gb2cli_type_mapping[fd->components->type] 
...
class GIGABASE_DLL_ENTRY dbCLI { 
  private:
    ...
    static int calculate_varying_length(char_t const* tableName, int& nFields, cli_field_descriptor* columns);
    static dbTableDescriptor* create_table_descriptor(dbDatabase* db, 
                                                      dbTable* table, char_t const* tableName, 
                                                      int nFields, int nColumns, 
  ...
  public:
    ...
    int create_statement(int session, char_t const* sql);
    ...
    int describe(int session, char_t const* table, cli_field_descriptor** fields);
    int show_tables(int session, cli_table_descriptor** tables);

    int match_columns(char_t const* table_name, statement_desc* stmt);

    

    int create_table(int session, char_t const* tableName, int nColumns, 
                     cli_field_descriptor* columns);

    int alter_table(int session, char_t const* tableName, int nColumns, 
                    cli_field_descriptor* columns);

    int drop_table(int session, char_t const* tableName);

    int alter_index(int session, char_t const* tableName, char_t const* fieldName, int newFlags);
    ...
    int prepare_query(int session, char_t const* query);
    int execute_query(int statement, int for_update, void* record_struct, va_list params);
    int insert_struct(int session, char_t const* table_name, void* record_struct, cli_oid_t* oid);

    static int create_table(dbDatabase* db, char_t const* tableName, int nColumns, 
                            cli_field_descriptor* columns);

    static int alter_table(dbDatabase* db, char_t const* tableName, int nColumns, 
                            cli_field_descriptor* columns);

    static int alter_index(dbDatabase* db, char_t const* tableName, char_t const* fieldName, int newFlags);
