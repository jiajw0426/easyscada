...
class dbFile { 
  ...
  public:
    ...
    // Create backup file
    //
    int    create(char_t const* name, int flags = no_buffering);
    //
    // Open database file and create file mapping object 
    //
    int    open(char_t const* fileName, char_t const* sharedName,
                int flags, size_t initSize, bool replicationSupport);
    
    void*  getAddr() const { return mmapAddr; }
    size_t getSize() const { return mmapSize; } 
    int    setSize(size_t size, char_t const* sharedName, bool initialize = true);
