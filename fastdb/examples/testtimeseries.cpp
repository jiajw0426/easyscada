...
int main(int argc, char* argv[])
{
    ...
    if (db.open(_T("timeseries"))) {
        ...
        for (int i = 0; i < N_QUOTES; i += QUOTE_BUF_SIZE) { 
            ...
            int n = proc.getInterval(stockId, i, i+QUOTE_BUF_SIZE-1, quotes, QUOTE_BUF_SIZE);
            assert(n == QUOTE_BUF_SIZE);
            for (int j = 0; j < n; j++) {
                assert(quotes[j].timestamp == i + j);
                assert(quotes[j].open == float(float(random(10000))/100));
                assert(quotes[j].close == float(float(random(10000))/100));
