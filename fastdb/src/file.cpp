                 ...
                 size_t initSize, bool replicationSupport)
{
    ...
    this->flags = flags;
#ifndef DISKLESS_CONFIGURATION
    if (_tcscmp(fileName, _T("/dev/zero")) == 0) { 
...
#endif
#if defined(NO_MMAP)
    if (fileSize < mmapSize && (flags & read_only) == 0) { 
...
int dbFile::create(const char_t* name, int flags)
{
    fh = CreateFile(name, GENERIC_READ|GENERIC_WRITE, 0, FASTDB_SECURITY_ATTRIBUTES, CREATE_ALWAYS, 
                    ((flags & no_buffering) ? FILE_FLAG_NO_BUFFERING : 0) | FILE_FLAG_SEQUENTIAL_SCAN, NULL); 
    if (fh == INVALID_HANDLE_VALUE) {
        return GetLastError();
    }
    mh = NULL;
...
#ifdef NO_MMAP
    if ((flags & no_sync) == 0 && !FlushFileBuffers(fh)))
#else
    if ((flags & no_sync) == 0 && !FlushViewOfFile(mmapAddr, mmapSize))
#endif
    {
        return GetLastError();
    }    
...
static int writeAll(int fd, void const* buf, size_t size) {
    char* src = (char*)buf;
    int rc;
    int written = 0;
...
bool dbFile::write(size_t pos, void const* ptr, size_t size) 
{
    if ((size_t)lseek(fd, pos, SEEK_SET) != pos
        || (size_t)writeAll(fd, mmapAddr + pos, size) != size) 
    { 
        dbTrace("Failed to save page to the disk, position=%ld, size=%ld, error=%d\n",
...
int dbFile::write(void const* buf, size_t& writtenBytes, size_t size)
{  
    long rc = writeAll(fd, (char*)buf, size);
    if (rc < 0) { 
        writtenBytes = 0;
        return errno;
    }
...
#if !defined(DISKLESS_CONFIGURATION) && !defined(REPLICATION_SUPPORT) && !defined(NO_FLUSH_ON_COMMIT)
#ifdef NO_MMAP
    if ((flags & no_sync) == 0 && msync(mmapAddr, mmapSize, MS_SYNC) != ok) 
#else
    if ((flags & no_sync) == 0 && fsync(fd) != ok) 
#endif
    { 
        return errno;
    }
