        ...
        { 
            ...
            if (fd->components->type == dbField::tpString) { 
                ...
                if (cb != NULL) { 
                    char* src = cb->ptr + 4;
                    while (--len >= 0) { 
                        int n = (wcslen((wchar_t*)src)+1)*sizeof(wchar_t);
                        offs += n;
                        src += n;
                    }
                } else { 
    ...
    for (cb = stmt->columns; cb != NULL; cb = cb->next) { 
        ...
        if (cb->cliType == cli_decimal) {
            ...
            if (cb->cliType == cli_array_of_string) { 
                ...
                while (--len >= 0) { 
                    int n = (wcslen((wchar_t*)data) + 1)*sizeof(wchar_t); 
                    offs += n;
                    data += n;
                }
            } else if (cb->cliType == cli_array_of_decimal) { 
...
bool dbServer::put_db_online(dbSession* session, char_t * msg) //put_db_online is APA added
{
	...
	if(!db->opened)
	{
		...
		char_t dbFileName[256];
		char_t databaseName[256];
		databaseName[0] = '\0';
		dbFileName[0] = '\0';
		...
		//put online the db 	
		if (!db->open(WC_STRING(databaseName), WC_STRING(dbFileName)))  
		{
