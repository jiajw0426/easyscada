...
inline unsigned dbHashTable::wstringHashFunction(wchar_t* key, int keylen) 
{
    unsigned h;
#ifdef IGNORE_CASE
    for (h = 0; --keylen >= 0;) { 
                         ...
                         oid_t rowId, int type, int sizeofType, int offs, size_t nRows)
{
    ...
    if (type == dbField::tpString) { 
        ...
        int len = ((dbVarying*)key)->size - 1;
        key = record + ((dbVarying*)key)->offs;
        hashkey = wstringHashFunction((wchar_t*)key, len);
    } else {  
...
void dbHashTable::find(dbDatabase* db, oid_t hashId, dbSearchContext& sc)
{
    ...
    if (sc.type == dbField::tpString) { 
        ...
        keylen = wcslen((wchar_t*)sc.firstKey);
        hashkey = wstringHashFunction((wchar_t*)sc.firstKey, keylen);
    } else {  
        ...
        if (item->hash == hashkey) { 
            ...
            switch (sc.type) { 
              case dbField::tpString:
                equals = keylen == ((dbVarying*)(rec + sc.offs))->size - 1
#ifdef IGNORE_CASE
                    && stricmp(sc.firstKey, 
                               (char*)rec+((dbVarying*)(rec+sc.offs))->offs) == 0
#else
                    && memcmp(sc.firstKey, rec+((dbVarying*)(rec+sc.offs))->offs, keylen) == 0;
#endif
                break;
              case dbField::tpWString:
                equals = keylen == ((dbVarying*)(rec + sc.offs))->size - 1
#ifdef IGNORE_CASE
                    && wcscasecmp((wchar_t*)sc.firstKey, 
                                  (wchar_t*)((char*)rec+((dbVarying*)(rec+sc.offs))->offs)) == 0
#else
                    && memcmp(sc.firstKey, rec+((dbVarying*)(rec+sc.offs))->offs, keylen*sizeof(wchar_t)) == 0;
#endif
                break;
             default:
                equals = sc.comparator(sc.firstKey, rec + sc.offs, keylen) == 0; 
            }
            if (equals) { 
                if (!sc.condition 
