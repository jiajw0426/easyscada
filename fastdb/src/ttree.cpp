...
#ifdef IGNORE_CASE
#define wcscmp(x, y) wcscasecmp(x, y)
#define wcsncmp(x, y, n) wcsncasecmp(x, y, n)
#endif
...
bool dbTtreeNode::prefixSearch(dbDatabase* db, dbSearchContext& sc)
{
    ...
    prefcmp_t prefcmp = (sc.type == dbField::tpString) ? (prefcmp_t)prefmbscmp : (prefcmp_t)prefwcscmp;
...
bool dbTtreeNode::find(dbDatabase* db, dbSearchContext& sc)
{
    ...
    } else if (sc.type == dbField::tpWString) { 
        if (sc.firstKey != NULL) { 
            rec = (char*)db->getRow(item[0]);
            diff = wcscmp((wchar_t*)sc.firstKey, (wchar_t*)(rec+((dbVarying*)(rec+sc.offs))->offs));
            if (diff >= sc.firstKeyInclusion) {     
                rec = (char*)db->getRow(item[n-1]);
                diff = wcscmp((wchar_t*)sc.firstKey, 
                              (wchar_t*)(rec+((dbVarying*)(rec+sc.offs))->offs));
                if (diff >= sc.firstKeyInclusion) { 
                    if (right != 0) { 
                        return ((dbTtreeNode*)db->get(right))->find(db, sc); 
                    } 
                ...
                while (r < n) { 
                    rec = (char*)db->getRow(item[r]);
                    if ((sc.lastKey != NULL 
                         && wcscmp((wchar_t*)(rec + ((dbVarying*)(rec+sc.offs))->offs),
                                   (wchar_t*)sc.lastKey) >= sc.lastKeyInclusion) || 
                        (sc.prefixLength != 0 
                         && memcmp(rec + ((dbVarying*)(rec+sc.offs))->offs,
                                   sc.firstKey,
                                   sc.prefixLength*sizeof(wchar_t)) != 0))
                    { 
                        return false;
                    }                        
                    if (!sc.condition 
                         ...
                         void* key, int type, int sizeofType, dbUDTComparator comparator, int offs)
{
    ...
    int diff = (type == dbField::tpString)
        ...
        : (type == dbField::tpWString)
           ? wcscmp((wchar_t*)key, (wchar_t*)(rec + ((dbVarying*)(rec+offs))->offs))
    ...
    } else if (type == dbField::tpWString) { 
        while (l < r)  {
            int i = (l+r) >> 1;
            rec = (char*)db->getRow(node->item[i]);
            diff = wcscmp((wchar_t*)key, (wchar_t*)(rec + ((dbVarying*)(rec+offs))->offs));
            if (diff > 0) { 
                l = i + 1;
            } else { 
                r = i;
