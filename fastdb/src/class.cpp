...
dbFieldDescriptor* dbFieldDescriptor::find1(char const* name)
{
          ...
          case dbField::tpWString:
            {
                ...
                if (str != NULL) { 
                    ...
                    offs += (((dbVarying*)(dst+fd->dbsOffs))->size = wcslen(str) + 1)*sizeof(wchar_t);
                } else { 
                    *(wchar_t*)(dst + offs) = '\0';
                    ((dbVarying*)(dst+fd->dbsOffs))->size = 1;
                    offs += sizeof(wchar_t);
    ...
    do { 
        ...
        *nextFieldLink = field;
        ...
        if (field->type != dbField::tpStructure) { 
            field->dbsOffs = fixedSize = DOALIGN(fixedSize, dbsFieldAlignment);
    ...
    for (fd = inverseFields; fd != NULL; fd = fd->nextInverseField) { 
        ...
        if (refTable == NULL) { 
            ...
            sprintf(msg, "Failed to locate referenced table %s", 
                    fd->refTableName != NULL ? fd->refTableName : fd->components->refTableName);
                                                      ...
                                                      int&        attr)
{
           ...
           && strncmp((char*)field + field->name.offs, prefix, prefixLen) == 0)
    {
        ...
        fd->appOffs = appSize = DOALIGN(appSize, appFieldAlignment);
        ...
        *nextFieldLink = fd;
        ...
        if (fd->type == dbField::tpArray || fd->type == dbField::tpStructure) {
            ...
            if (fd->type == dbField::tpStructure) {
                ...
                fd->appOffs = saveAppSize = DOALIGN(saveAppSize, appAlignment);
                appSize += saveAppSize;
            } else { 
                appSize = saveAppSize;
            ...
            if (fd->type == dbField::tpString) { 
                ...
                fd->components = new dbFieldDescriptor("[]");
                if (sizeof(wchar_t) == 2) { 
                    fd->components->type = fd->components->appType = dbField::tpInt2;
                    fd->components->dbsSize = fd->components->appSize = fd->components->alignment = 2; 
                } else { 
                    fd->components->type = fd->components->appType = dbField::tpInt4;
                    fd->components->dbsSize = fd->components->appSize = fd->components->alignment = 4; 
                }
            }
            field += 1;
        }
