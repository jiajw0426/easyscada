...
#include "..\fastdb\inc\sockio.h"
#include "..\fastdb\inc\repsock.h"
#include "..\fastdb\inc\sync.h"
#include "..\fastdb\inc\cli.h"
#include "..\fastdb\inc\cliproto.h"
...
struct session_desc { 
    ...
    session_desc(int id, session_desc* next) {
		...
		IT_IT("session_desc::session_desc(int id, session_desc* next) in cli_fastdb.cpp");
    ...
    session_desc() 
	{
		IT_IT("session_desc::session_desc() in cli_fastdb.cpp");
	}
};
...
class descriptor_table { 
  ...
  public:
    descriptor_table() {
		...
		IT_IT("descriptor_table::descriptor_table in cli_fastdb.cpp");
...
int cli_column_fdb(int         statement,
...
int cli_array_column_fdb(int            statement,
                     char const*    column_name,
                     int            var_type,
                     void*          var_ptr,
                     cli_column_set set,
                     cli_column_get get)
{
    return cli_array_column_ex_fdb(statement, column_name, var_type, var_ptr, 
...
int cli_fetch_fdb(int statement, int for_update)
{
...
int cli_update_fdb(int statement)
{
    int rc = cli_send_columns_fdb(statement, cli_cmd_update);
    if (rc == cli_ok) { 
        int4 response;
        statement_desc* s = statements.get(statement);
        s->updated = true;
...
int cli_get_first_fdb(int statement)
{
    return cli_get_fdb(statement, cli_cmd_get_first);
}

int cli_get_last_fdb(int statement)
{
    return cli_get_fdb(statement, cli_cmd_get_last);
}

int cli_get_next_fdb(int statement)
{
    return cli_get_fdb(statement, cli_cmd_get_next);
}

int cli_get_prev_fdb(int statement)
{
    return cli_get_fdb(statement, cli_cmd_get_prev);
}

int cli_skip_fdb(int statement, int n)
{
    return cli_get_fdb(statement, cli_cmd_skip, n);
}

int cli_seek_fdb(int statement, cli_oid_t oid)
{
    return cli_get_fdb(statement, cli_cmd_seek, oid);
}
...
int cli_freeze_fdb(int statement)
{
    return send_receive(statement, cli_cmd_freeze);
}

int cli_unfreeze_fdb(int statement)
{
    return send_receive(statement, cli_cmd_unfreeze);
}

cli_oid_t cli_get_oid_fdb(int statement)
{
    statement_desc* s = statements.get(statement);
    if (s == NULL) { 
        return 0;
...
static int cli_send_command_fdb(int session, int statement, int cmd)
{
    session_desc* s = sessions.get(session);
    if (s == NULL) { 
        return cli_bad_descriptor;
...
int cli_close_cursor_fdb(int statement)
{
    return cli_ok;
}
...
int cli_commit_fdb(int session)
{
    return cli_send_command_fdb(session, 0, cli_cmd_commit);
}

int cli_precommit_fdb(int session)
{
    return cli_send_command_fdb(session, 0, cli_cmd_precommit);
}

int cli_abort_fdb(int session)
{
    return cli_send_command_fdb(session, 0, cli_cmd_abort);
}

int cli_remove_fdb(int statement)
{
    statement_desc* s = statements.get(statement);
    if (s == NULL) { 
        return cli_bad_descriptor;
    ...
    return cli_send_command_fdb(s->session->id, s->id, cli_cmd_remove);
}

int cli_remove_current_fdb(int statement)
{
    statement_desc* s = statements.get(statement);
    if (s == NULL) { 
        return cli_bad_descriptor;
    ...
    return cli_send_command_fdb(s->session->id, s->id, cli_cmd_remove_current);
}
...
int cli_describe_layout_fdb(int session, char const* table, cli_field_layout** fields, int* rec_size)
{
...
int cli_create_fdb(char_t const* databaseName,
...
int cli_create_table_fdb(int session, char const* tableName, int nColumns, 
                     cli_field_descriptor* columns)
{
    return cli_update_table(cli_cmd_create_table, session, tableName, nColumns, columns);
}

int cli_alter_table_fdb(int session, char const* tableName, int nColumns, 
                     cli_field_descriptor* columns)
{
...
cli_error_handler cli_set_error_handler_fdb(int session, cli_error_handler new_handler, void* context)
{
    return NULL;
}

int cli_create_replication_node_fdb(int         nodeId,
                                int         nServers,
                                char*       nodeNames[],
...
int cli_attach_fdb(int session)
{
    return cli_not_implemented;
}

int cli_detach_fdb(int session, int detach_mode)
{
    return cli_not_implemented;
}

void cli_free_memory_fdb(int, void* ptr)
{
    free(ptr);
}

int cli_get_database_state_fdb(int session, cli_database_monitor* monitor)
{
    return cli_not_implemented;
}

void cli_set_trace_function_fdb(cli_trace_function_t) 
{ 
}

int cli_prepare_query_fdb(int session, char const* query)
{
    return cli_not_implemented;
}

int cli_execute_query_fdb(int statement, int for_update, void* record_struct, ...)
{
    return cli_not_implemented;
}

int cli_execute_query_ex_fdb(int statement, int for_update, void* record_struct, int n_params, int* param_types, void** param_value)
{
    return cli_not_implemented;
}

int cli_insert_struct_fdb(int session, char const* table_name, void* record_struct, cli_oid_t* oid)
{
    return cli_not_implemented;
}

int cli_get_field_size_fdb(cli_field_descriptor* fields, int field_no)
{
    return sizeof_type[fields[field_no].type];
}


int cli_get_field_offset_fdb(cli_field_descriptor* fields, int field_no)
{
    int offs = 0;
    int size = 0;
...
int cli_xml_export_fdb(int session, FILE* out)
{
    return cli_not_implemented;
}

int cli_xml_import_fdb(int session, FILE* in)
{
    return cli_not_implemented;
}

int cli_backup_fdb(int session, char const* file_name, int compactify)
{
    return cli_not_implemented;
}

int cli_schedule_backup_fdb(int session, char const* file_name, int period)
{
    return cli_not_implemented;
}
...
int put_db_offline_fdb(int session)
{
	return cli_send_command_fdb(session, 0, cli_cmd_put_db_offline); //FIX ME fare lato server
}
