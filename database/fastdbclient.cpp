...
int Fastdb_Thread::allocate_bindings()
{
	IT_IT("Fastdb_Thread::allocate_bindings");
	...
	if(server_is_connected && db_online)
	{
		...
		QSNumberOfTables = cli_show_tables_fdb(session_handle, &tables);
		...
		for(int i = 0; i < QSNumberOfTables; i++)
		{
			...
			nCols = cli_describe_fdb(session_handle, (char_t const*)(tables[i].name), &fields);

			if(nCols < 0)
			{
				IT_COMMENT1("cli_describe001 failed with code %d", nCols);	
		...
		value_asciiz = new char_t*[max_numCol];
		...
		updated_field_name = new char_t*[max_numCol];
		if(updated_field_name == NULL) return -1;
		new_field_value = new char_t*[max_numCol];
		if(new_field_value == NULL) return -1;
		selected_field_name = new char_t*[max_numCol];
		if(selected_field_name == NULL) return -1;
		selected_field_name2 = new char_t*[max_numCol];
		if(selected_field_name2 == NULL) return -1;
		
		for(i = 0; i < max_numCol; i++)
		{
			value_asciiz[i] = new char_t[MAX_LENGHT_OF_STRING];
			if(value_asciiz[i] == NULL) return -1;
			updated_field_name[i] = new char_t[MAX_LENGHT_OF_STRING];
			if(updated_field_name[i] == NULL) return -1;
			new_field_value[i] = new char_t[MAX_LENGHT_OF_STRING];
			if(new_field_value[i] == NULL) return -1;
			selected_field_name[i] = new char_t[MAX_LENGHT_OF_STRING];
			if(selected_field_name[i] == NULL) return -1;
			selected_field_name2[i] = new char_t[MAX_LENGHT_OF_STRING];
...
void Fastdb_Thread::deallocate_bindings()
{
	IT_IT("Fastdb_Thread::deallocate_bindings");
...
bool Fastdb_Thread::SetQuery(const QString &s)
{	
	IT_IT("Fastdb_Thread::SetQuery");
	
	if(Done) 
	{
		strcpy(fasdb_query, (const char_t*)(s.stripWhiteSpace())); // set the query 
		Trigger = true; // go 
...
void Fastdb_Thread::run()
{	
	IT_IT("Fastdb_Thread::run");
	...
	IT_COMMENT1("Fastdb_Thread is Running, connected to %s", (const char_t*)(Parent->GetName()));
	
	while(true)
	{
		IT_COMMENT("Fastdb_Thread is Waiting....");
		...
		if(fExit)
		{
			IT_COMMENT("Fastdb_Thread is Exiting ....");
			deallocate_bindings();
			m_hevtEnd.signal();
			return; //terminate the thread
		}
		
		IT_COMMENT("Fastdb_Thread Past Wait Flag");
		if(Trigger)
		{
			if(pLastResult)
			{
			...
			db_message = QObject::tr("Query: [") + QString(fasdb_query) + "]  ";
			
			{
				///START TRANSACTION/////////////////////////////////////////////////////////
								
				IT_COMMENT1("Query: %s", fasdb_query);
				...
				switch(main_token) 
				{
					case tkn_select:
					{
						...
						IT_COMMENT1("Modified query: %s", (const char_t *)query_buf);
						statement = cli_statement_fdb(session_handle, (const char_t *)query_buf);		
						...
						if (statement >= 0)
						{
							...
							nCols = cli_describe_fdb(session_handle, (char_t const*) (table_name), &fields);
							if(nCols < 0)
							{
								IT_COMMENT1("cli_describe002 failed with code %d", nCols);	
								error_message += QString("cli_describe failed with code ") + QString(error_list[nCols*(-1)].name) + " ";
								fFail = true;
								unrecovableError = true;
								break;
							...
							for(j = 0; j < nCols; j++)
							{
								...
								switch(fields[j].type)
								{
									...
									{
										rc = cli_column_fdb(statement, fields[j].name, cli_asciiz, NULL, value_asciiz[j]);
										if(rc != cli_ok)
										{
									...
									case cli_real8:
									{
										rc = cli_column_fdb(statement, fields[j].name, cli_real8, NULL, &value_real8[j]);
										if(rc != cli_ok)
										{
									...
									case cli_real4:
									{
										rc = cli_column_fdb(statement, fields[j].name, cli_real4, NULL, &value_real4[j]);
										if(rc != cli_ok)
										{
									...
									case cli_int4:
									{
										rc = cli_column_fdb(statement, fields[j].name, cli_int4, NULL, &value_int4[j]);
										if(rc != cli_ok)
										{
									...
									case cli_oid:
									{
										rc = cli_column_fdb(statement, fields[j].name, cli_oid, NULL, &value_oid[j]);
										if(rc != cli_ok)
										{
									...
									case cli_int8:
									{
										rc = cli_column_fdb(statement, fields[j].name, cli_int8, NULL, &value_int8[j]);
										if(rc != cli_ok)
										{
							...
							rc = cli_fetch_fdb(statement, cli_view_only);
							if (rc < 0) 
							{
							...
							for (k = 0;((rc = cli_get_next_fdb(statement)) == cli_ok) ; k++) 
							{
							...
							rc = cli_commit_fdb(session_handle);
				    ...
				    case tkn_insert:
					{
						...
						if (expect("into", tkn_into) && expect("table name", tkn_ident)) 
						{
							...
							if(nCols < 0)
							{
								IT_COMMENT1("cli_describe003 failed with code %d", nCols);	
								error_message += QString("cli_describe failed with code ") + QString(error_list[nCols*(-1)].name) + " ";
								fFail = true;
								unrecovableError = true;
								break;
							...
							if (expect("(", tkn_lpar)) 
							{
								...
								statement = cli_statement_fdb(session_handle, (const char_t *)NewQuery);		
								if (statement >= 0)
								{
									/////////read values////////////////
									int tkn;
										...
										{	
											if(!strcmp(fields[j].name, updated_field_name[tk]))
											{
												switch(fields[j].type)
												{
													case cli_asciiz:
													{
														strcpy(value_asciiz[j], (const char_t*)new_field_value[tk]);
														//IT_COMMENT2("field %s cli_asciiz: %s", fields[j].name, value_asciiz[j]);
													...
													case cli_real8:
													{
														value_real8[j] = (cli_real8_t)atof((const char_t*)new_field_value[tk]);
														//IT_COMMENT2("field %s cli_real8: %lf", fields[j].name, value_real8[j]);
														tk = nNewFields; //serve a finire il ciclo!
													...
													case cli_real4:
													{
														value_real4[j] = (cli_real4_t)atof((const char_t*)new_field_value[tk]);
														//IT_COMMENT2("field %s cli_real4: %lf", fields[j].name, value_real4[j]);
														tk = nNewFields; //serve a finire il ciclo!
													...
													case cli_int4:
													{
														value_int4[j] = (cli_int4_t)atoi((const char_t*)new_field_value[tk]);
														//IT_COMMENT2("field %s cli_int4:  %d", fields[j].name, value_int4[j]);
														tk = nNewFields; //serve a finire il ciclo!
													...
													case cli_oid:
													{
														value_oid[j] = (cli_oid_t)atol((const char_t*)new_field_value[tk]);
														//IT_COMMENT2("field %s cli_oid:  %d", fields[j].name, value_oid[j]);
														tk = nNewFields; //serve a finire il ciclo!
													}
													break;
													case cli_int8:
													{
														value_int8[j] = (cli_int8_t)_atoi64((const char_t*)new_field_value[tk]);
														//IT_COMMENT2("field %s cli_int8:  %d", fields[j].name, value_int8[j]);
														tk = nNewFields; //serve a finire il ciclo!
													}
													break;
									...
									rc = cli_insert_fdb(statement, &oid);
									...
									else
									{
										...
										rc = cli_precommit_fdb(session_handle);
										if(rc != cli_ok)  
										{
					...
					case tkn_delete:
					{
						...
						if (expect("from", tkn_from) && expect("table name", tkn_ident)) 
						{
							...
							if (statement >= 0)
							{
								...
								rc = cli_fetch_fdb(statement, cli_for_update);
								...
								{
									////////////begin patch del 19-07-09//////////////////////
									while ((rc = cli_get_prev_fdb(statement)) == cli_ok) 
									{ 
										//IT_COMMENT1("record # %d", k);
									}
									...
									if (rc != cli_not_found) 
									{ 
										IT_COMMENT1("cli_get_prev failed with code %d", rc);
										error_message += QString("cli_get_prev failed with code ") + QString(error_list[rc*(-1)].name) + " ";
										fFail = true;
										break;
									}
									/////////end patch del 19-07-09/////////////////////////////////////

									rc = cli_remove_fdb(statement);
						...
						if (expect("table name", tkn_ident)) 
						{
							...
							if(expect("set", tkn_set))
							{
								...
								if (statement >= 0)
								{
									...
									if(nCols < 0)
									{
										IT_COMMENT1("cli_describe004 failed with code %d", nCols);	
										error_message += QString("cli_describe failed with code ") + QString(error_list[nCols*(-1)].name) + " ";
										fFail = true;
										unrecovableError = true;
										break;
											...
											for(int jk = 0; jk < nUpdatedFields; jk++)
											{
												if(!strcmp(fields[j].name, updated_field_name[jk]))
												{
													switch(fields[j].type)
													{
														case cli_asciiz:
														{
															strcpy(value_asciiz[j], (const char_t*)new_field_value[jk]);
														...
														case cli_real8:
														{
															value_real8[j] = (cli_real8_t)atof((const char_t*)new_field_value[jk]);
															jk = nUpdatedFields; //serve a terminare il ciclo
															IT_COMMENT2("field %s cli_real8: %lf", fields[j].name, value_real8[j]);
														}
														break;
														case cli_real4:
														{
															value_real4[j] = (cli_real4_t)atof((const char_t*)new_field_value[jk]);
															jk = nUpdatedFields; //serve a terminare il ciclo
															IT_COMMENT2("field %s cli_real4: %lf", fields[j].name, value_real4[j]);
														}
														break;
														case cli_int4:
														{
															value_int4[j] = (cli_int4_t)atoi((const char_t*)new_field_value[jk]);
															jk = nUpdatedFields; //serve a terminare il ciclo
															IT_COMMENT2("field %s cli_int4:  %d", fields[j].name, value_int4[j]);
														}
														break;
														case cli_oid:
														{
															value_oid[j] = (cli_oid_t)atol((const char_t*)new_field_value[jk]);
															jk = nUpdatedFields; //serve a terminare il ciclo
															IT_COMMENT2("field %s cli_oid:  %d", fields[j].name, value_oid[j]);
														}
														break;
														case cli_int8:
														{
															value_int8[j] = (cli_int8_t)_atoi64((const char_t*)new_field_value[jk]);
															jk = nUpdatedFields; //serve a terminare il ciclo
															IT_COMMENT2("field %s cli_int8:  %d", fields[j].name, value_int8[j]);
														}
														break;
...
/*
					case tkn_create:
					{
						...
						rc = cli_create_table_apa(session_handle, fasdb_query);
						if(rc != cli_ok)
						{
...
*/
							...
							case tkn_index:
							{
								if (expect("on", tkn_on) && expect("table name", tkn_ident)) 
								{
	   							   ...
	   							   if (expect(".", tkn_dot) && expect("field name", tkn_ident)) 
								   {
										...
										rc = cli_alter_index_fdb(session_handle, (char_t const*) table_name, (char_t const*)fieldName, cli_indexed);
										if(rc != cli_ok)
										{
							...
							case tkn_table:
							{
								if(expect("table name", tkn_ident)) 
								{
									...
									while (tkn == tkn_comma) 
									{
										...
										strcpy((char*)fields[nFields].name, name);
										...
										switch (scan()) 
										{
											...
											case tkn_reference:
											{
												...
												{
													fields[nFields].refTableName = new char_t[strlen(name)+1];
													strcpy((char*)fields[nFields].refTableName, name);
													type = cli_oid;
													break;
												}
												else
									...
									if (tkn == tkn_rpar) 
									{
										rc = cli_create_table_fdb(session_handle, (char_t const*) table_name, nColumns, fields);
	   									if(rc != cli_ok)
										{
					...
					case tkn_alter:
					{
						...
						if(expect("table", tkn_table) && expect("table name", tkn_ident)) 
						{
									...
									case tkn_reference:
									{
										if(expect("to", tkn_to) && expect("referenced table name", tkn_ident)) 
										{
											...
											if(tkn == tkn_inverse) 
											{
												...
												fields[nFields].inverseRefFieldName = new char_t[STRLEN(name)+1];
							...
							if (tkn == tkn_rpar) 
							{
								rc = cli_alter_table_fdb(session_handle, (char_t const*) table_name, nColumns, fields);
 								if(rc != cli_ok)
								{
						...
						switch (scan())
						{
							...
							case tkn_index:
							{
								if (expect("on", tkn_on) && expect("table name", tkn_ident)) 
								{
	   							   ...
	   							   if (expect(".", tkn_dot) && expect("field name", tkn_ident)) 
								   {
										...
										rc = cli_alter_index_fdb(session_handle, (char_t const*) table_name, (char_t const*)fieldName, 0);
										if(rc != cli_ok)
										{
											IT_COMMENT1("cli_alter_index failed with code %d", rc);
											error_message += QString("cli_alter_index failed with code ") + QString(error_list[rc*(-1)].name) + " ";
							...
							case tkn_table:
							{
								if(expect("table name", tkn_ident)) 
								{
									...
									rc = cli_drop_table_fdb(session_handle, (char_t const*) table_name);
									if(rc != cli_ok)
									{
					...
					*/
					case tkn_open:
					{
						if (expect("database file name", tkn_sconst)) 
						{
							if (db_online) 
							{
								rc = put_db_offline_fdb(session_handle);
								if(rc != cli_ok)
								{
							...
							rc = put_db_online_fdb(session_handle, buf);
							if(rc != cli_ok)
							{
					...
					*/
					case tkn_rollback:
					{
						rc = cli_abort_fdb(session_handle);
						if(rc != cli_ok)  
						{
					...
					case tkn_show:
					{
						...
						ntables = cli_show_tables_fdb(session_handle, &tables);
						if(ntables < 0)
						{
					...
					case tkn_describe:
					{
						...
						if(tk == tkn_ident)
						{
							...
							nfields = cli_describe_fdb(session_handle, buf, &fields);
							if(nfields < 0)
							{
								IT_COMMENT1("cli_describe005 failed with code %d", nfields);	
...
QString Fastdb_Thread::nested_select(const QString &table_name)
{
	IT_IT("Fastdb_Thread::nested_select");
	...
	if (readCondition()) 
	{
		...
		strcpy(fasdb_query,(const char_t*) new_condition);
		pos = 0;
		...
		if(token == tkn_in)
		{
			...
			if(main_token == tkn_select)
			{
				...
				IT_COMMENT1("Modified second query: %s", (const char_t *)query_buf);
				statement = cli_statement_fdb(session_handle, (const char_t *)query_buf);		
				...
				if (statement >= 0)
				{
					...
					nCols = cli_describe_fdb(session_handle, (char_t const*) (table_name2), &fields);
					if(nCols < 0)
					{
						IT_COMMENT1("cli_describe006 failed with code %d", nCols);	
						error_message += QString("cli_describe failed with code ") + QString(error_list[nCols*(-1)].name) + " ";
						fFail = true;
						unrecovableError = true;
						QString a = "";
					...
					for (int k = 0;((rc = cli_get_next_fdb(statement)) == cli_ok); k++) 
					{
						//IT_COMMENT1("record # %d", k);
...
void Fastdb_Thread::unget(int ch) {
...
void Fastdb_Thread::error(char_t const* msg)
{
	IT_IT("Fastdb_Thread::error");
	IT_COMMENT(msg);
	fFail = true;
	error_message += QString("Error -> [") + QString(msg) + QString("] in query ->") + QString(fasdb_query);
}

int Fastdb_Thread::scan()
{
	int i, ch, digits;
    ...
    switch (ch) {
		...
		{
			char_t car[50];
			sprintf(car,"Invalid symbol %c", (char_t)ch);
			error(car);
			return tkn_error;
		}
    }
...
bool Fastdb_Thread::expect(char_t const* expected, int token)
{
    int tkn = scan();
...
bool Fastdb_Thread::readCondition()
{
    int i, ch;
...
int Fastdb_Thread::unupported_types(int type)
{
	IT_IT("Fastdb_Thread::unupported_types");
