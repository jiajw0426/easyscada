...
/*!
  ...
  \param name Name
*/
PThermometer::PThermometer(QWidget *parent, const char *name)
        : QWidget(parent,name)
{
    // set default values
    dMin=-10.0;
...
*/
int PThermometer::setMinValue(double newMinValue)
{
    dMin=newMinValue;
    if(dThreshold<dMin)
        dThreshold=dMin;
...
*/
int PThermometer::setMaxValue(double newMaxValue)
{
    int retCode=0;
    if(newMaxValue<dMin)
    {
...
/*!
  ...
  \sa BorderStyle
*/
int PThermometer::setBorderStyle(BorderStyle newBorderStyle)
{
    //check input??
...
*/
int PThermometer::setPrecision(int newPrecision)
{
    int retCode=0;
...
*/
int PThermometer::setNumTicks(int newNumTicks)
{
    int retCode=0;
    if(newNumTicks<0)
    {
...
*/
int PThermometer::setThreshold(double newThreshold)
{
    int retCode=0;
...
*/
void PThermometer::paintEvent(QPaintEvent *event)
{
...
/*!
  \brief Repaint the widget
*/
void PThermometer::drawThermometer(QPainter *painter)
{
    ...
    painter->drawLine(w/2-w/16,h*3/5+h/10,w/2+w/16,h*3/5+h/10); //cancel??
    ...
    // tank
    QBrush brush=QBrush(red, SolidPattern);
...
QSize PThermometer::sizeHint()
{
    return minimumSizeHint();
}


QSize PThermometer::minimumSizeHint()
{
...
*/
void PThermometer::mouseReleaseEvent(QMouseEvent * e)
{
	if(e->button() == Qt::RightButton)
	{
		QString name = this->name();

		emit RightClicked(QString("PThermometer"), name);
	}
}